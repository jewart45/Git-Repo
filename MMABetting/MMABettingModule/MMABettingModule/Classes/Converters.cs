...
namespace MMABettingModule.Classes
{
    public static class Converters
    {
        ...
        public static List<OddsInfo> ToOddsInfo(this ISelection f, Event ev)
        {
            ...
            {
                list.Add(run.Odds != "-" ?
                new OddsInfo() { FightDate = ev.Date, FightName = ev.Name.Trim(), Name = run.NameNoSpaces, OddsValue = (long)Convert.ToDecimal(run.Odds), Percent = (long)Convert.ToDecimal(run.PercentChanceDecimal), SelectionID = run.SelectionID, EventType = f.Name }
                : new OddsInfo() { FightDate = ev.Date, FightName = ev.Name.Trim(), Name = run.NameNoSpaces, OddsValue = 0, Percent = 0, SelectionID = run.SelectionID, EventType = f.Name });
            }
